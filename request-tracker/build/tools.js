...
async function makeRTRequest(endpoint, options = {}) {
...
async function createTicket(ticketData) {
    try {
        ...
        if (ticketData.requestor)
            payload.Requestor = ticketData.requestor;
        if (ticketData.cc)
            payload.Cc = ticketData.cc;
        if (ticketData.content)
            payload.Content = ticketData.content;
 ...
 */
async function addComment(ticketId, content, type = "comment") {
    try {
        const payload = {
            Content: content,
            ContentType: "text/plain",
        };
        const endpoint = type === "correspond"
 ...
 */
async function getTicketLinks(ticketId) {
    try {
